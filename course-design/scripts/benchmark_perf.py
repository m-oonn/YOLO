...
print("\n1. Health Check")
bench(
    "GET /health", lambda: requests.get(f"{BASE.replace('/api', '')}/health", timeout=5)
)

# 2. Detection status
print("\n2. Detection Status")
bench(
    "GET /detection/status", lambda: requests.get(f"{BASE}/detection/status", timeout=5)
)
...
bench(
    ...
    lambda: requests.post(
        f"{BASE}/detection/start",
        json={"source": "0", "config": "configs/default.yaml"},
        timeout=30,
    ),
)
time.sleep(5)
bench(
    "GET /detection/status (running)",
    lambda: requests.get(f"{BASE}/detection/status", timeout=5),
)
bench(
    "POST /detection/stop", lambda: requests.post(f"{BASE}/detection/stop", timeout=10)
)
