...
def make_skeleton_at(
    ...
    keypoints.extend(
        [
            SkeletonKeypoint(
                x=head_x - shoulder_width, y=shoulder_y, confidence=conf
            ),  # left shoulder
            SkeletonKeypoint(
                x=head_x + shoulder_width, y=shoulder_y, confidence=conf
            ),  # right shoulder
            SkeletonKeypoint(
                x=head_x - shoulder_width * 1.2, y=elbow_y, confidence=conf
            ),  # left elbow
            SkeletonKeypoint(
                x=head_x + shoulder_width * 1.2, y=elbow_y, confidence=conf
            ),  # right elbow
            SkeletonKeypoint(
                x=head_x - shoulder_width * 1.3, y=wrist_y, confidence=conf
            ),  # left wrist
            SkeletonKeypoint(
                x=head_x + shoulder_width * 1.3, y=wrist_y, confidence=conf
            ),  # right wrist
    ...
    keypoints.extend(
        [
            SkeletonKeypoint(
                x=head_x - hip_width, y=hip_y, confidence=conf
            ),  # left hip
            SkeletonKeypoint(
                x=head_x + hip_width, y=hip_y, confidence=conf
            ),  # right hip
            SkeletonKeypoint(
                x=head_x - hip_width * 0.8, y=knee_y, confidence=conf
            ),  # left knee
            SkeletonKeypoint(
                x=head_x + hip_width * 0.8, y=knee_y, confidence=conf
            ),  # right knee
            SkeletonKeypoint(
                x=head_x - hip_width * 0.7, y=ankle_y, confidence=conf
            ),  # left ankle
            SkeletonKeypoint(
                x=head_x + hip_width * 0.7, y=ankle_y, confidence=conf
            ),  # right ankle
...
class TestRunningDetectionBaselines:
    ...
    def test_walking_speed_no_false_positive(self):
        ...
        assert len(running_events) == 0, (
            "Walking speed should not trigger running detection"
        )
    ...
    def test_sprinting_speed_high_confidence(self):
        ...
        assert running_events[0].confidence >= 0.7, (
            "Sprinting should have high confidence"
        )
...
class TestRunningDetectionMetrics:
    ...
    def test_confusion_matrix_simulation(self):
        ...
        for _scenario_name, (speed_kmh, should_detect) in scenarios.items():
            rule = SkeletonRunningRule(cfg)
            buffer = SkeletonFrameBuffer()
            px_per_km = 100.0
            speed_px_s = speed_kmh * px_per_km / 3.6
