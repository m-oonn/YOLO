...
class SkeletonSequenceBuffer:
    ...
    def clear_old(self, max_age_frames: int = 300) -> None:
        ...
        current_max = max(
            (seq[-1].frame_index for seq in self._buffers.values() if seq), default=0
        )
        to_remove = []
...
class ActionAnalyzer:
    ...
    def _match_template(
        self, features: ActionFeatures, template: BehaviorTemplate
    ) -> float:
    ...
    def _calculate_chaotic_score(
        ...
        for seq in [seq1, seq2]:
            ...
            for i in range(2, len(seq)):
                ...
                v1 = np.sqrt(
                    (center_i1[0] - center_i2[0]) ** 2
                    + (center_i1[1] - center_i2[1]) ** 2
                )
                v2 = np.sqrt(
                    (center_i[0] - center_i1[0]) ** 2
                    + (center_i[1] - center_i1[1]) ** 2
                )
                accel = abs(v2 - v1)
    ...
    def _detect_crowd_behavior(
        ...
        if density > 0.5:  # Threshold for crowd
            return {
                "behavior": "crowd",
                "track_ids": [
                    sk.track_id for sk in skeletons if sk.track_id is not None
                ],
    ...
    def clear_sequences(self) -> None:
        ...
        self.sequence_buffer = SkeletonSequenceBuffer(
            max_length=self.sequence_buffer.max_length
        )
