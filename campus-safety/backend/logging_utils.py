...
# ──────────────────────────────────────────────────────────
# 【后端API】logging_utils.py — 结构化日志
# 依赖：仅标准库（json, logging, uuid）
# 被调用：backend/main.py（请求追踪中间件）
# 核心职责：
#   ① 为每个HTTP请求生成唯一 X-Request-ID
#   ② JSON格式结构化日志（方便ELK等日志平台采集）
#   ③ 请求耗时记录（method + path + status + IP + duration）
# ──────────────────────────────────────────────────────────
