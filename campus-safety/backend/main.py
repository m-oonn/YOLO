...
# ──────────────────────────────────────────────────────────
# 【后端API】main.py — FastAPI 应用入口
# 上游依赖：core/（整个核心引擎）
# 下游调用：前端通过 HTTP/WebSocket 连接此服务
# 核心职责：
#   ① 创建 FastAPI 应用实例
#   ② 注册 8 个 API 路由（detection/events/alarms/cameras/config/mllm）
#   ③ 配置 CORS 中间件（允许前端跨域访问）
#   ④ 应用生命周期管理（启动预热模型，关闭清理资源）
#   ⑤ 全局异常处理（自定义异常 → HTTP状态码映射）
#   ⑥ 请求追踪中间件（X-Request-ID + 请求日志）
