...
# ──────────────────────────────────────────────────────────
# 【后端API】alarm_singleton.py — AlarmEngine 单例
# 依赖：core/alarm_engine.py
# 被调用：pipeline.py（注入报警引擎）；API路由（查询报警）
# 核心职责：
#   ① 懒初始化 AlarmEngine（首次访问时创建，含SQLite存储）
#   ② 进程级单例（确保 pipeline 和 API 路由共享同一报警引擎）
#   ③ 优雅关闭（应用退出时清理）
# ──────────────────────────────────────────────────────────
