...
# ──────────────────────────────────────────────────────────
# 【后端API路由】detection.py — 检测控制端点
# 上游依赖：backend/detection_manager.py
# 下游调用：前端 MonitorView.vue 通过此API控制检测
# 路由前缀：/api/detection
# 核心端点：
#   POST /start    — 启动检测（source=摄像头/视频路径）
#   POST /stop     — 停止检测
#   GET  /status   — 查询运行状态（FPS/帧数/事件数/模型信息）
#   GET  /progress — 查询视频处理进度
#   POST /upload   — 上传视频文件到服务器
#   GET  /stream.mjpg — MJPEG实时视频流（前端img标签直接播放）
#   WS   /stream   — WebSocket性能数据推送（FPS/帧数实时更新）
# ──────────────────────────────────────────────────────────
