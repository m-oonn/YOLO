...
# ──────────────────────────────────────────────────────────
# 【后端API路由】events.py — 事件查询端点
# 上游依赖：backend/store.py（EventsStore单例）
# 下游调用：前端 EventsView.vue 查询事件数据
# 路由前缀：/api/events
# 核心端点：
#   GET /           — 分页查询事件（支持类型/时间/搜索筛选）
#   GET /stats      — 事件统计（按类型汇总数量）
#   GET /types      — 列出所有事件类型
#   DELETE /all     — 清空全部事件
#   GET /{id}/snapshot — 获取事件的快照图片
# ──────────────────────────────────────────────────────────
