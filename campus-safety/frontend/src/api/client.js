/**
 * ┌──────────────────────────────────────────┐
 * │ 【前端UI】api/client.js — API 客户端       │
 * │ 职责：封装所有后端API调用（axios）          │
 * │ 包含6个API模块：                           │
 * │   camerasAPI — 摄像头列表                  │
 * │   detectionAPI — 检测启停/状态/视频上传    │
 * │   eventsAPI — 事件查询/统计/删除           │
 * │   alarmsAPI — 报警查询/确认/处理           │
 * │   mllmAPI — MLLM状态/配置                  │
 * │   configAPI — 规则/设置/禁区读写           │
 * │   archivesAPI — 视频片段下载               │
 * │ 特性：请求去重(dedup) + 短期缓存(2秒TTL)   │
 * └──────────────────────────────────────────┘
