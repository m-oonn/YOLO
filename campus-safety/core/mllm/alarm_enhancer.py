...
# ──────────────────────────────────────────────────────────
# 【MLLM子系统】alarm_enhancer.py — MLLM报警二次验证
# 依赖：inference_engine.py（推理引擎）
# 被调用：alarm_engine.py（报警触发时二次确认）
# 核心职责：
#   ① 传入报警事件的快照 → VLM判断"这是真的打架/跌倒吗？"
#   ② 冷却机制（10秒内同类不重复验证，避免GPU过载）
#   ③ 返回验证结论（confirmed / false_alarm / uncertain）
#   ④ shadow_mode 模式下仅记录验证结果，不修改报警状态
# ──────────────────────────────────────────────────────────
