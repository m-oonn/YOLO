...
# ──────────────────────────────────────────────────────────
# 【MLLM子系统】inference_engine.py — 多模态推理引擎
# 依赖：transformers（Qwen2-VL / Florence-2 / SmolVLM）
# 被调用：scene_describer.py, alarm_enhancer.py
# 核心职责：
#   ① 自动选择可用的VLM后端（Qwen2-VL优先，Florence-2备选）
#   ② 加载模型 + 图片预处理 + 文本生成
#   ③ 支持批处理和流式生成加速
# 支持的模型：Qwen2-VL-2B, Florence-2, SmolVLM-256M
# ──────────────────────────────────────────────────────────
