...
# ──────────────────────────────────────────────────────────
# 【核心引擎】events_store.py — 事件持久化存储（SQLite WAL模式）
# 上游依赖：db_base.py（SQLite基类）, constants.py
# 下游调用：pipeline.py 每帧事件写入；backend/api/events.py 查询
# 核心职责：
#   ① 事件CRUD（insert/query/delete/stats）
#   ② feature_blob 存储（CLIP特征向量，支持图文检索）
#   ③ 批量写入优化（EVENT_FLUSH_BATCH=50）
#   ④ 自动VACUUM清理（每1000事件触发增量清理）
# 数据流：Event → INSERT → SQLite ← SELECT → 前端查询
# ──────────────────────────────────────────────────────────
