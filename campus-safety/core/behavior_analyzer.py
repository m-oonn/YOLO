...
# ──────────────────────────────────────────────────────────
# 【核心引擎】behavior_analyzer.py — 骨架行为分析（5种行为）
# 上游依赖：skeleton.py（骨架数据）, pose_features.py（特征计算）
# 下游调用：pipeline.py 调用并融合结果到事件系统
# 核心职责：基于人体关键点的精细行为识别 ——
#   ① 骨架奔跑 — 髋部速度 > 10km/h
#   ② 骨架跌倒 — 躯干角度 > 40° + 头部高度 < 0.3 + 速度变化
#   ③ 骨架打架 — 手腕速度 > 1.5m/s + 肢体频率 > 2.5Hz
#   ④ 骨架聚集 — 人群密度 > 3.5 人/m²
#   ⑤ 骨架入侵 — 关键点进入禁区
# 这是 BBox规则引擎的"补充维度"，两者互补提高准确率
# ──────────────────────────────────────────────────────────
