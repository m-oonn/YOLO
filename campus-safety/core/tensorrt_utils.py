...
# ──────────────────────────────────────────────────────────
# 【核心引擎】tensorrt_utils.py — TensorRT推理加速（可选）
# 依赖：tensorrt, onnx, torch
# 被调用：pipeline.py（如果GPU支持TensorRT则自动加速）
# 核心职责：
#   ① YOLO PyTorch模型 → ONNX导出 → TensorRT引擎
#   ② FP16/INT8精度的推理加速（2-4x速度提升）
#   ③ 引擎缓存（避免每次启动重复编译）
# 当前状态：可选模块，需要NVIDIA GPU + TensorRT SDK
# ──────────────────────────────────────────────────────────
