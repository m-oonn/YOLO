...
# ──────────────────────────────────────────────────────────
# 【核心引擎】jetson_utils.py — Jetson边缘设备适配
# 依赖：jetcam, jetson-inference（仅Jetson平台）
# 被调用：pipeline.py（如果在Jetson上运行则自动适配）
# 核心职责：
#   ① 检测Jetson设备型号（Nano/TX2/Xavier/Orin）
#   ② Jetson专用GPU内存管理（统一内存）
#   ③ CSI摄像头适配
# 当前状态：仅Jetson平台需要，PC上无实际作用
# ──────────────────────────────────────────────────────────
