...
from core.rules import Detection, RulesEngine, Event
from core.config import FallRule
...
def make_skeleton_at(track_id: int, x: float, y: float,
                     height: float = 170.0, conf: float = 0.9,
                     torso_angle: float = 0.0, head_y: float | None = None) -> Skeleton:
    ...
    keypoints.extend([
        SkeletonKeypoint(x=shoulder_x - shoulder_width, y=shoulder_y, confidence=conf),
        SkeletonKeypoint(x=shoulder_x + shoulder_width, y=shoulder_y, confidence=conf),
        SkeletonKeypoint(x=shoulder_x - shoulder_width * 1.2, y=shoulder_y + height * 0.15, confidence=conf),
        SkeletonKeypoint(x=shoulder_x + shoulder_width * 1.2, y=shoulder_y + height * 0.15, confidence=conf),
        SkeletonKeypoint(x=shoulder_x - shoulder_width * 1.3, y=shoulder_y + height * 0.25, confidence=conf),
        SkeletonKeypoint(x=shoulder_x + shoulder_width * 1.3, y=shoulder_y + height * 0.25, confidence=conf),
    ])
    ...
    keypoints.extend([
        ...
        SkeletonKeypoint(x=x + hip_width, y=y, confidence=conf),
        SkeletonKeypoint(x=x - hip_width * 0.8, y=y + height * 0.25, confidence=conf),
        SkeletonKeypoint(x=x + hip_width * 0.8, y=y + height * 0.25, confidence=conf),
        SkeletonKeypoint(x=x - hip_width * 0.7, y=y + height * 0.5, confidence=conf),
        SkeletonKeypoint(x=x + hip_width * 0.7, y=y + height * 0.5, confidence=conf),
    ])
...
def _person(track_id, x1, y1, x2, y2, conf=0.9):
    return Detection(track_id=track_id, class_id=0, conf=conf, x1=x1, y1=y1, x2=x2, y2=y2)
...
class TestSkeletonFallTruePositives:
    ...
    def test_forward_fall_detected(self):
        ...
        for i in range(15):
            ...
            sk = make_skeleton_at(1, 200, 200 + head_drop, torso_angle=angle, head_y=200 - 80 + head_drop)
            buffer.append(t, [sk])
            e = rule.detect([sk], t, 15 + i + 1, buffer)
            events.extend(e)
        
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) >= 1, f"Forward fall should be detected, got {len(fall_events)} events"
    ...
    def test_backward_fall_detected(self):
        ...
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) >= 1, f"Backward fall should be detected, got {len(fall_events)} events"
    ...
    def test_lateral_fall_detected(self):
        ...
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) >= 1, f"Lateral fall should be detected, got {len(fall_events)} events"
    
    def test_rapid_fall_emergency_detection(self):
        """Rapid fall with very high head velocity should trigger emergency."""
        cfg = make_skeleton_config(min_duration_s=0.1, debounce_s=2.0, fall_velocity_threshold=0.5)
        rule = SkeletonFallRule(cfg)
        buffer = SkeletonFrameBuffer()
        ...
        for i in range(10):
            ...
            sk = make_skeleton_at(1, 200, 200 + head_drop * 0.5, torso_angle=60.0, head_y=120 + head_drop)
            buffer.append(t, [sk])
...
class TestSkeletonFallFalsePositives:
    ...
    def test_bending_no_false_positive(self):
        ...
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) == 0, f"Bending should not trigger fall, got {len(fall_events)} events"
    ...
    def test_squatting_no_false_positive(self):
        ...
        for i in range(60):
            ...
            sk = make_skeleton_at(1, 200, 200 + y_offset, torso_angle=10.0, head_y=120 + y_offset)
            buffer.append(t, [sk])
            e = rule.detect([sk], t, i + 1, buffer)
            events.extend(e)
        
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) == 0, f"Squatting should not trigger fall, got {len(fall_events)} events"
    ...
    def test_sitting_down_slowly_no_false_positive(self):
        ...
        for i in range(90):
            ...
            sk = make_skeleton_at(1, 200, 200 + y_offset, torso_angle=angle, head_y=120 + y_offset)
            buffer.append(t, [sk])
            e = rule.detect([sk], t, i + 1, buffer)
            events.extend(e)
        
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) == 0, f"Sitting slowly should not trigger fall, got {len(fall_events)} events"
    ...
    def test_walking_no_false_positive(self):
        ...
        fall_events = [e for e in events if e.event_type == "fall"]
        assert len(fall_events) == 0, f"Walking should not trigger fall, got {len(fall_events)} events"
...
class TestFallDetectionLatency:
    ...
    def test_rapid_fall_latency(self):
        ...
        for i in range(60):
            ...
            if t < fall_start_time:
                ...
                sk = make_skeleton_at(1, 200, 200 + head_drop * 0.3, torso_angle=angle, head_y=120 + head_drop)
        ...
        if fall_detected_at is not None:
            ...
            assert latency < 0.5, f"Rapid fall detection latency {latency:.2f}s exceeds 0.5s"
    ...
    def test_slow_fall_latency(self):
        ...
        if fall_detected_at is not None:
            latency = fall_detected_at - fall_start_time
            assert latency < 2.0, f"Slow fall detection latency {latency:.2f}s exceeds 2.0s"
...
class TestFallDetectionMetrics:
    ...
    def test_confusion_matrix(self):
        ...
        for scenario, is_fall in scenarios.items():
            rule = SkeletonFallRule(cfg)  # Fresh rule per scenario to avoid debounce contamination
            buffer = SkeletonFrameBuffer()
            events = []
            ...
            for i in range(90):  # 3 seconds for better detection
                ...
                if scenario == "forward_fall":
                    if i < 15:
                        ...
                        head_drop = (i - 15) * 8.0
                        sk = make_skeleton_at(1, 200, 200 + head_drop * 0.5, torso_angle=angle, head_y=120 + head_drop)
                    ...
                    sk = make_skeleton_at(1, 200, 200 + y_off, torso_angle=10.0, head_y=120 + y_off)
                
                elif scenario == "sitting_slowly":
                    y_off = min(80.0, i * 1.5)
                    angle = min(20.0, i * 0.3)
                    sk = make_skeleton_at(1, 200, 200 + y_off, torso_angle=angle, head_y=120 + y_off)
                    ...
                    sk = make_skeleton_at(1, 200, 200, torso_angle=angle, head_y=120 + i * 0.8)
